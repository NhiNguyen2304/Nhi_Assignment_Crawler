 ...
 * @author admin
 */
public class VietBaoCrawler1 extends BaseCrawler {
    ...
    public boolean getGoldFromURL(String baseUrl, String date, int type) {
        ...
        try {
            ...
            if (!document.isEmpty()) {
                ...
                Logger.getLogger(VietBaoCrawler1.class.getName()).log(Level.SEVERE, null, "Null Document");
            }

            //System.out.println("Document " + changingDoc);
            if (!wellFormedDoc.isEmpty()) {
                urlVietBao = baseUrl + getURL(wellFormedDoc, date);
                if (type == 2) {
    ...
    public boolean sTAXPaeserForGold(String document, String date) throws UnsupportedEncodingException, XMLStreamException, IOException {
        ...
        while (eventReader.hasNext()) {
            ...
            if (event.isStartElement()) {
                ...
                if (aTag.equals(tagName)) {
                    String goldName = "";
                    Attribute attrGoldName = startElement.getAttributeByName(new QName("title"));
                    if (attrGoldName != null) {
                        goldName = attrGoldName.getValue().trim();
                        dto.setTypeOfGold(goldName);
